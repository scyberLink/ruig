...
class Fetcher {
  ...
  addListenerForUrl(
    fetchOptions: IAnyObject,
    listener: (arg0: unknown) => void,
  ...
  removeListener(id: string) {
    if (!id) throw Error('Id required to remove listener')
  ...
  async fetch(
    options: IAnyObject | string,
    returnType = FetcherResponseType.JSON,
  ): Promise<IFetchData | AxiosResponse | Blob> {
    let url
